...
public class Interpreter2{
	...
	boolean bound(String x) {
		return list.currentDlist.params.containsKey(x);
	}
	
	Node getVal(String x) {
		return list.currentDlist.params.get(x);
	}
	...
	public static Node eval(Node root){
		...
		if (command.equals("PLUS")){
			if (length(root) != 3){
				System.out.print("PLUS undefined when length != 3");
				System.exit(1);
			}
			return plus(eval(car(cdr(root))), eval(car(cdr(cdr(root)))));
		}

		if (command.equals("MINUS")){
			if (length(root) != 3){
				System.out.print("MINUS undefined when length != 3");
				System.exit(1);
			}
			return minus(eval(car(cdr(root))), eval(car(cdr(cdr(root)))));
		}

		if (command.equals("TIMES")){
			if (length(root) != 3){
				System.out.print("TIMES undefined when length != 3");
				System.exit(1);
			}
			return times(eval(car(cdr(root))), eval(car(cdr(cdr(root)))));
			
		}

		if (command.equals("LESS")){
			if (length(root) != 3){
				System.out.print("LESS undefined when length != 3");
				System.exit(1);
			}
			return less(eval(car(cdr(root))), eval(car(cdr(cdr(root)))));			
		}

		if (command.equals("GREATER")){
			if (length(root) != 3){
				System.out.print("GREATER undefined when length != 3");
				System.exit(1);
			}
			return greater(eval(car(cdr(root))), eval(car(cdr(cdr(root)))));			
		}

		if (command.equals("EQ")){
			if (length(root) != 3){
				System.out.print("EQ undefined when length != 3");
				System.exit(1);
			}
			return eq(eval(car(cdr(root))), eval(car(cdr(cdr(root)))));			
		}

		if (command.equals("ATOM")){
			if (length(root) != 2){
				System.out.print("ATOM undefined when length != 2");
				System.exit(1);
			}
			
			return atom(eval(car(cdr(root))));
		}
		
		if (command.equals("INT")){
			if (length(root) != 2){
				System.out.print("INT undefined when length != 2");
				System.exit(1);
			}
			return intt(eval(car(cdr(root))));
		}

		if (command.equals("NULL")){
			if (length(root) != 2){
				System.out.print("NULL undefined when length != 2");
				System.exit(1);
			}
			return nulll(eval(car(cdr(root))));
		}

		if (command.equals("CAR")){
			if (length(root) != 2){
				System.out.print("CAR undefined when length != 2");
				System.exit(1);
			}
			return car(eval(car(cdr(root))));
		}

		if (command.equals("CDR")) {
			if (length(root) != 2){
				System.out.print("CDR undefined when length != 2");
				System.exit(1);
			}
			return cdr(eval(car(cdr(root))));
		}

		if (command.equals("CONS")){
			if (length(root) != 3){
				System.out.print("CONS undefined when length != 3");
				System.exit(1);
			}
			return cons(eval(car(cdr(root))), eval(car(cdr(cdr(root)))));
		}

		if (command.equals("QUOTE")){
			if (length(root) != 2){
				System.out.print("QUOTE undefined when length != 2");
				System.exit(1);
			}
			return car(cdr(root));
		}
		...
		if (command.equals("COND")){
			...
			for (int i = 0; i < iterations; i++){
				...
				if (!eval(car(car(copy))).key.equals("NIL")){
					return eval(car(cdr(car(copy))));
				} else {
					copy = cdr(copy);
				}
			}
		...
		if (command.equals("DEFUN")) {
			...
			System.out.println(car(cdr(root)).key);
			
			}



		System.out.println("CAR of tree is not among allowed strings.");
		System.exit(1);
